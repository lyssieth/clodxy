...
complete -c clodxy -f -l backend -d "Backend to use" -a "(jq -r '.backends | keys[]' 2>/dev/null ~/.config/clodxy/config.json)"
complete -c clodxy -f -l model -d "Model to use" -a "(jq -r '.backends | map(.models | keys) | add | unique[]' 2>/dev/null ~/.config/clodxy/config.json)"
